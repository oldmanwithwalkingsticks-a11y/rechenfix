  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      <div className="relative">
        <svg className={`absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none ${grosse === 'gross' ? 'w-6 h-6' : 'w-5 h-5'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="absolute left-0 right-0 top-full mt-2 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 z-50 overflow-hidden">
          {ergebnisse.map(r => (
            ...
            >
              ...
              <span className="text-xs text-gray-400 dark:text-gray-500 shrink-0 ml-auto">{r.kategorie}</span>
            </button>
          ))}
        </div>
      )}
