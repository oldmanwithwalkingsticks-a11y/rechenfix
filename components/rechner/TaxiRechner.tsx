  ...
  return (
    <div>
      <div className="space-y-4 mb-6">
        <div>
          <label htmlFor="stadt" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Stadt / Region</label>
          <select
            id="stadt"
            value={stadt}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Strecke (km)</label>
            <NummerEingabe value={strecke} onChange={setStrecke} placeholder="10" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Wartezeit (Minuten, optional)</label>
        ...
        <>
          ...
          <div className="result-box mb-4">
            <div className="text-center mb-3">
              <p className="text-white/70 text-sm mb-1">Geschätzter Fahrpreis — {ergebnis.stadt.name}</p>
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl overflow-hidden mb-4">
            <div className="px-4 py-3 bg-gray-50 dark:bg-gray-700/50">
              <p className="text-sm font-semibold text-gray-700 dark:text-gray-200">Preisaufschlüsselung</p>
            </div>
            <div className="divide-y divide-gray-100 dark:divide-gray-700">
              <div className="flex justify-between px-4 py-3 text-sm">
                <span className="text-gray-600 dark:text-gray-400">Grundgebühr ({ergebnis.nacht ? 'Nacht' : 'Tag'})</span>
                <span className="font-medium text-gray-800 dark:text-gray-200">{fmt(ergebnis.grundgebuehr)} €</span>
              </div>
              <div className="flex justify-between px-4 py-3 text-sm">
                <span className="text-gray-600 dark:text-gray-400">Strecke ({ergebnis.strecke.toLocaleString('de-DE')} km)</span>
                <span className="font-medium text-gray-800 dark:text-gray-200">{fmt(ergebnis.streckenkosten)} €</span>
              </div>
              {ergebnis.wartekosten > 0 && (
                <div className="flex justify-between px-4 py-3 text-sm">
                  <span className="text-gray-600 dark:text-gray-400">Wartezeit ({ergebnis.wartezeit} Min.)</span>
                  <span className="font-medium text-gray-800 dark:text-gray-200">{fmt(ergebnis.wartekosten)} €</span>
                </div>
              )}
              <div className="flex justify-between px-4 py-3 text-sm font-bold">
                ...
                <span className="text-primary-600 dark:text-primary-400">{fmt(ergebnis.fahrpreis)} €</span>
              </div>
              <div className="flex justify-between px-4 py-3 text-sm">
                <span className="text-gray-600 dark:text-gray-400">Trinkgeld (10 %)</span>
                <span className="font-medium text-gray-800 dark:text-gray-200">{fmt(ergebnis.trinkgeld)} €</span>
              </div>
              <div className="flex justify-between px-4 py-3 text-sm font-bold">
                <span className="text-gray-700 dark:text-gray-200">Gesamt mit Trinkgeld</span>
                <span className="text-primary-600 dark:text-primary-400">{fmt(ergebnis.gesamtMitTrinkgeld)} €</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/30 rounded-xl p-4 mb-4">
            <p className="text-sm font-semibold text-blue-800 dark:text-blue-300 mb-2">Tarif {ergebnis.stadt.name}</p>
            <div className="grid grid-cols-2 gap-2 text-xs text-blue-700 dark:text-blue-400">
              <p>Grundgebühr: {fmt(ergebnis.nacht ? ergebnis.stadt.grundNacht : ergebnis.stadt.grundTag)} €</p>
              <p>Wartezeit: {fmt(ergebnis.stadt.warteMinute)} €/Min.</p>
              <p>Bis {ergebnis.stadt.kmSchwelle} km: {fmt(ergebnis.nacht ? ergebnis.stadt.kmNacht1 : ergebnis.stadt.kmTag1)} €/km</p>
              <p>Ab {ergebnis.stadt.kmSchwelle + 1} km: {fmt(ergebnis.nacht ? ergebnis.stadt.kmNacht2 : ergebnis.stadt.kmTag2)} €/km</p>
            </div>
          </div>
