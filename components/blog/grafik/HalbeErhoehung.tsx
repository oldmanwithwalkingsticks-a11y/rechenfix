 ...
 * Legende rechts neben dem Raster, nicht an den Quadraten — bei 22 px
 * Kantenlänge wäre dort kein Text unterzubringen.
  ...
  return (
    <figure className="my-8">
      <svg width="100%" viewBox="0 0 680 400" role="img" xmlns="http://www.w3.org/2000/svg" className="rounded-xl text-gray-900 dark:text-gray-100">
        ...
        {legende.map((l, i) => (
          <g key={l.text}>
            <rect x="360" y={110 + i * 62} width={KANTE} height={KANTE} rx="3" className={`f-${l.klasse}`} />
            <text x="394" y={126 + i * 62} fontSize="14" fontWeight="600" fill="currentColor">{l.titel}</text>
            <text x="394" y={146 + i * 62} fontSize="12" fill="#9ca3af">{l.text}</text>
          </g>
        ))}

        <rect className="schnitt" x="360" y="240" width="288" height="66" rx="8" strokeWidth="1" />
        <text x="378" y="266" fontSize="12" fill="#9ca3af">Amtlich ausgewiesen wurde</text>
        <text x="378" y="292" fontSize="20" fontWeight="600" fill="currentColor">+1,9 % im Bundesdurchschnitt</text>

        <line x1="24" y1="356" x2="656" y2="356" stroke="#d1d5db" strokeWidth="1" />
        <text x="24" y="374" fontSize="12" fill="#9ca3af">Wer zu einem Salon der rechten Hälfte ging, zahlte sechs Prozent mehr. Wer zur linken ging, gar nichts.</text>
        <text x="24" y="392" fontSize="12" fill="#9ca3af">Den Durchschnitt von 1,9 Prozent bezahlte niemand.</text>
      </svg>
    </figure>
  );
}
