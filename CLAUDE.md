...
> **TikTok-Takt (W52):** TikTok postet nur jeden zweiten Tag, gerechnet in ganzen Tagen ab `TIKTOK_TAKT_START` (06.08.2026) in `lib/social/utils.ts`. Grund ist das PostPeer-Kontingent von 20 Credits im Monat. IG/FB bleiben täglich. Der Takt wird über `tiktokEnabled()` in `lib/social/state.ts` gesteuert und wirkt darüber auf `platformsForSlug` — die Zuständigkeitslogik ist damit tagesabhängig. Wird der Takt geändert, ist **nur** `TIKTOK_TAKT_START` bzw. der Modulo-Wert anzufassen, nichts an der Rotationslogik.

> **Getrennte Cron-Zeiten (W53):** IG/FB posten um 17 UTC über `/api/cron/social-post`, TikTok um 05 UTC über `/api/cron/social-post-tiktok`. Grund ist der 24-Stunden-Creator-Cap von PostPeers API-Client, der sich über alle Kunden dieses Anbieters teilt — 17 UTC fällt in dessen Stoßzeit. Die Verschiebung ist ein **Messaufbau**: Bleiben die `reached_active_user_cap`-Fehler auch um 05 UTC, ist der Cap rund um die Uhr voll und nur ein Anbieterwechsel hilft. `vercel.json` enthält bewusst **keine** Query-Parameter im Cron-Pfad, weil Vercel diese nicht dokumentiert. Der Plattformfilter läuft über `publishToBothPlatforms(force, dryRun, platforms)`; der manuelle Re-Trigger `/api/cron/social-post?platforms=…` kann die Zuständigkeit überschreiben.
