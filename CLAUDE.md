...
- **Welle 19 (Goldstandard-Content):** Migration der Rechner auf modulare `contentBloecke` (≥11 Bausteine, ~1.500–1.560 W, eigenes Leitformat pro Rechner, `config.quellen`, KEIN `text`-Block >170 W). Stand 20.06.2026: **69 Rechner** im contentBloecke-Goldstandard-Set (gemessen via `scripts/check-contentbloecke-struktur.mjs`). Details + Tranchen-Historie in [docs/audit-arbeitspapiere/welle-status-historie.md](docs/audit-arbeitspapiere/welle-status-historie.md). Block-Granularität + Self-Check: siehe [SKILL.md](.claude/skills/rechner-builder/SKILL.md) Abschnitt „Block-Granularität".
