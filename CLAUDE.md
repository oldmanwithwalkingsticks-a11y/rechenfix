...
> **Getrennte Cron-Zeiten (W53):** IG/FB posten um 17 UTC über `/api/cron/social-post`, TikTok um 05 UTC über `/api/cron/social-post-tiktok`. Grund ist der 24-Stunden-Creator-Cap von PostPeers API-Client, der sich über alle Kunden dieses Anbieters teilt — 17 UTC fällt in dessen Stoßzeit. Die Verschiebung war ein Messaufbau; sie half nicht, der Wechsel auf bundle.social (W59) schon. **W66:** Erster Post am 08.08.2026 sichtbar, kein Fehlereintrag. Auswertung ab 20.08. nach `docs/audit-arbeitspapiere/arbeitspapier-tiktok-messvorschrift-2026-08-08.md`. PostPeer bleibt bis auf Weiteres als Rollback über `TIKTOK_PROVIDER=postpeer` erhalten — Variablen in Vercel stehen lassen. `vercel.json` enthält bewusst **keine** Query-Parameter im Cron-Pfad, weil Vercel diese nicht dokumentiert. Der Plattformfilter läuft über `publishToBothPlatforms(force, dryRun, platforms)`; der manuelle Re-Trigger `/api/cron/social-post?platforms=…` kann die Zuständigkeit überschreiben.
