...
## Projekt-Status (Stand 20.06.2026)

- **206 eigenständige Rechner in 10 Kategorien** (Alltag 24, Finanzen 45, Gesundheit 17, Auto & Verkehr 15, Wohnen & Energie 25, Mathe & Schule 18, Arbeit & Recht 17, Kochen & Ernährung 15, Sport & Fitness 15, Technik 15). **Gemessen 31.08.2026** aus `lib/rechner-config/<kategorie>.ts`, je Datei mit `grep -cE '^\s*slug:'` und gegengeprüft über `^\s*kategorieSlug:` — Vorsicht: `slug:` steht auf vier Leerzeichen, ein Muster mit fest zwei Leerzeichen liefert null Treffer und sieht aus wie ein Befund. Diese Zahlen sind handgepflegte Aggregate über eine generierte Struktur; sie altern mit jedem neuen Rechner und werden beim nächsten Bestandslauf neu gemessen, nicht fortgeschrieben. **Historisch:** Am 19.06.2026 (Welle 19) kam die **10. Kategorie „Technik"** dazu (`kategorieSlug: 'technik'`, Icon 💻), damals mit 5 Rechnern (internetgeschwindigkeit, datenmengen-umrechner, megapixel, stromverbrauch-geraete, bildschirmgroesse-ppi), und Sport wuchs von 2 auf 5 — beide sind seither weiter gewachsen. Frühere Verschiebung 26.04.2026 (Prompt 149a): arbeitslosengeld-rechner aus Arbeit nach Finanzen migriert (SSOT-Konsistenz pro Kategorie-Datei).
- **Welle 19 (Goldstandard-Content):** Migration der Rechner auf modulare `contentBloecke` (≥11 Bausteine, ~1.500–1.560 W, eigenes Leitformat pro Rechner, `config.quellen`, KEIN `text`-Block >170 W). Stand 21.06.2026: **93 Rechner** im contentBloecke-Goldstandard-Set (gemessen via `scripts/check-contentbloecke-struktur.mjs`; Tranchen bis t69 abgeschlossen). Details + Tranchen-Historie in [docs/audit-arbeitspapiere/welle-status-historie.md](docs/audit-arbeitspapiere/welle-status-historie.md). Block-Granularität + Self-Check: siehe [SKILL.md](.claude/skills/rechner-builder/SKILL.md) Abschnitt „Block-Granularität".
- **254 URLs** in der Sitemap, gemessen 31.08.2026 (`https://www.rechenfix.de/sitemap.xml`, HTTP 200, 45.253 B, 254 `<loc>`). Die Zerlegung folgt den Blöcken in [app/sitemap.ts](app/sitemap.ts) — ändert sich dort etwas, fällt die Abweichung am Block auf und nicht erst an der Summe: `staticPages` 3 (Root, `/aktualisierungen`, `/ki-rechner`) + `longTailPages` 6 (`/finanzen/2000-euro-brutto-netto` bis `/5000-euro-brutto-netto`) + `sonderseiten` 2 (`brutto-netto-tabelle`, `mindestlohn-netto`) + `kategoriePages` 10 + `metaPages` 9 (über uns, qualität, feedback, barrierefreiheit, impressum, datenschutz, ki-transparenz, nutzungsbedingungen, offline-nutzung) + `blogUebersicht` 1 + `blogArtikel` 17 + `rechnerPages` 206 = 254. Bewusst draußen: `/admin/affiliate-stats` (intern), `/social` (noindex Bio-Hub) und der Gerüstartikel `beispiel-artikel` (in `blogArtikel` herausgefiltert). **`/ki-rechner` steht seit 22.08.2026 drin** — der frühere Ausschluss (noindex, W15A.4) trifft nicht mehr zu. Rechner und Kategorien zieht der Generator dynamisch aus `lib/rechner-config/`; eine neue Kategorie-Datei wird automatisch erfasst.
...
## Config-Struktur
- Configs liegen aufgeteilt unter `lib/rechner-config/<kategorie>.ts` (`alltag.ts`, `finanzen.ts`, `gesundheit.ts`, `auto.ts`, `wohnen.ts`, `mathe.ts`, `arbeit.ts`, `kochen.ts`, `sport.ts`, `technik.ts`). Die alte Single-File `rechner-config.ts` existiert nicht mehr. Bei NEUER Kategorie: Config-Datei anlegen, in `index.ts` registrieren (Import + Spread ins `rechner`-Array + Objekt im `kategorien`-Array mit `einleitung`), Farbe in `lib/social/kategorie-farben.json` ergänzen (`verify-kategorie-slugs.ts` muss grün bleiben) und die hartkodierten Kategorie-Listen in `scripts/check-contentbloecke-wortzahl.mjs`, `scripts/check-contentbloecke-struktur.mjs` sowie `scripts/slug-drift-scan.mjs` (KATEGORIEN **und** PATTERN-Regex) nachziehen.
- **Neuer Rechner = Config-Slug + RechnerLoader-Eintrag + Component-File, immer alle drei.** Ein Config-Eintrag ohne passenden `components/rechner/RechnerLoader.tsx`-Eintrag (und zugehöriges Component-File) rendert eine **leere Rechner-Box** — der `slug-drift-scan` prüft diese Kopplung NICHT (er prüft nur hartkodierte Pfad-Strings). Slugs mit führender Ziffer (z. B. `1rm-rechner`) sind unproblematisch, weil Slugs nur als String-Keys/-Values genutzt werden; das Component-File trägt dann einen ausgeschriebenen Namen (`EinRmRechner.tsx`).
...
#### Vercel `ignoreCommand` — Doku-only-Pushes überspringen den Build (CPU-Spar)

`vercel.json` enthält einen `ignoreCommand`, der reine Doku-Pushes (nur `docs/**`, `*.md`, `.github/**`) vom Build ausschließt. **Wichtig (Lehre 14.06.2026):** Der Guard difft seit dem Fix gegen **`VERCEL_GIT_PREVIOUS_SHA`** (= letzter *erfolgreich deployter* Commit auf dem Branch), NICHT mehr nur gegen `HEAD^..HEAD`. Grund: Bei der Sub-Wellen-Konvention (Commit 1 Content, Commit 2 Doku) war die Branch-Spitze immer ein `docs`-Commit; die alte `HEAD^..HEAD`-Prüfung sah dann nur Doku und übersprang den Build **trotz Content im darunterliegenden Commit** — fünf W19-Goldstandard-Rechner (t06–t10) lagen so unbemerkt unbebaut auf `main`. Die Range-Prüfung gegen den letzten Deploy fängt jeden Content-Anteil im gesamten Push (und über mehrere übersprungene Deploys hinweg). Fällt `VERCEL_GIT_PREVIOUS_SHA` weg (erster Deploy / nicht exponiert), baut der Guard sicherheitshalber (`exit 1`). **Folge:** Content+Doku dürfen weiter getrennt committet werden; reine Doku-Syncs überspringen den Build weiterhin korrekt.

**Getrennt committen heißt nicht getrennt pushen (gemessen 06.09.2026).** Code- und Historie-Commit gehören in **einen** Push. Vercel legt eine Deployment **je Push** an, nicht je Commit, und der `ignoreCommand` vergleicht gegen den zuletzt ausgelieferten Stand, nicht gegen `HEAD^`. Die Deployment trägt deshalb den SHA des **Kopf**-Commits — bei dieser Konvention also den des Historie-Commits, obwohl der Code im vorangehenden Commit steht. Gemessen am Push `aaa6414..89e73a1`: genau eine Deployment, SHA `89e73a1`, Zustand READY; `92c46bf` mit der eigentlichen Code-Änderung hat nie eine eigene Deployment erhalten. Eine Commit-Nachricht, die behauptet, „der Code-Commit deployt", beschreibt die Mechanik falsch — gebaut wurde, weil `app/impressum/page.tsx` im Diff des **gesamten Pushes** lag. Wird der Historie-Commit dagegen **allein nachgeschoben**, nachdem der Code-Commit bereits ausgeliefert ist, ist sein Diff docs-only und der Build wird übersprungen; die Code-Änderung erreicht das Netz nicht. Genau das ist bei `0803d16` passiert (Deployment CANCELED) und war der Anlass für die `public/**`-Ergänzung in `194ed7a`.
...
- **157b** (Commit 566a095) — kuendigungsfrist + abfindung: § 169 SGB IX Schwerbehinderten-Mindestfrist + Integrationsamt; § 113 InsO Insolvenz-Höchstfrist 3 Mon; §§ 9, 10 KSchG gerichtliche Auflösung 12/15/18 Monatsverdienste; § 4 KSchG 3-Wo-Klagefrist (P3-A2-3, P3-A2-4, P3-A3-2, P3-A3-3) ✅
- **157c** (Commit d3eca80) — scheidungskosten: § 1565 Abs. 2 BGB Härtefall, VA-Mindest-VW 1.000 €, Folgesachen-Faustwert-Hinweis, VKH § 115 ZPO, Nr. 7002 VV RVG Auslagen-Cap (P3-A5-1..5) ✅
- **157d** (Commit e4810ec) — zugewinnausgleich + unterhalt: § 1379 BGB Auskunftsanspruch, § 1375 Abs. 2 BGB illoyale Vermögensminderung, § 1385 BGB vorzeitiger Ausgleich, „bereinigtes Netto"-Klarstellung (P3-A6-1..3, P3-A7-1) ✅
- **157e** (Commit 965c519) — ehegattenunterhalt: Beispiel mit Kindesunterhalt-Vorabzug, § 1573 Abs. 4 BGB Anschlussunterhalt, Halbteilung Nicht-Erwerb, § 1574 BGB Erwerbsobliegenheit, § 1577 BGB Anrechnung (P3-A10-1..5) ✅
- **157f** (Commit 6689668) — arbeitslosengeld: §§ 153/147/155 SGB III, Höchstsatz ~2.940 €/Monat, Steuerklasse-Stichtag 01.01., Nebenverdienst 165 € + 15-h-Schwelle (P3-A8-1..5) ✅
- **P3-B1** (Commit 7c2426b) — ueberstunden-Netto-Refactor: Pauschale 40-%-Steuerabzug-Schätzung im Vergütungs-Modus durch Mehrbetrag-Methode `Netto(Brutto+Vergütung) − Netto(Brutto)` via `berechneBruttoNetto`-SSOT ersetzt. UI im Modus „Vergütung" um Steuerklasse + Bundesland + Kirchensteuer-Inputs erweitert. Schließt P3-B1 aus dem Welle-2-Stufe-3-Arbeit-Block-B-Backlog ✅

### Doku-Sync 28.04.2026

- **157-doku-sync** (Commit a750bf4) — Doku-Sync für Item 157 in welle-status-historie: konsolidierter 157-Block mit 6 Sub-Commits 157a–f + Welle-3-Backlog-Update; 152c-Eintrag als geparkter Slot ergänzt ✅
...
- **W17A.1.A** (Commit e9dad20) — EXCLUDED_SLUGS in `lib/social/config.ts` (10 Phase-0-Top-10-Slugs alle gegen Repo verifiziert), SHUFFLE_SEED=17 (fest), `scripts/build-social-queue.ts` mit Mulberry32-RNG + Fisher-Yates, `lib/social/queue.json` (160 Slugs, deterministisch reproduzierbar) ✅
- **W17A.1.B** (Commit b25a8d5) — `state.ts` erweitert um `isSlugDone(slug)` + `markSlugDone(slug, dateBerlin)` + Key-Schema `social:done:{slug}`. Done-Marke wird gesetzt sobald MIND. eine Plattform Success (kein Re-Post bei Teil-Fehler) ✅
- **W17A.1.C** (Commit 77b489a) — Publisher-Komplett-Umbau: Modulo-Rotation raus, Queue-Iteration mit `pickNextSlug` + `resolveTodayPost` rein. `lib/social/posts.json` gelöscht, `lib/social/captions.json` initialisiert (leer). Cron-Endpoint angepasst: `slug` statt `postIndex`, Queue-erschöpft = 200 statt 503, Dry-Run liefert imageExists/captionExists/queueExhausted im JSON ✅
- **W17A.1.E** (Commit 072bf8b) — `scripts/social-caption-builder.ts` (lokal, Anthropic-API): liest queue.json + rechner-config, Modell `claude-sonnet-4-20250514` (Override via ANTHROPIC_MODEL), System-Prompt verlangt rohes JSON, resumable mit Write-Through. ~8–15 Min Laufzeit für 160 Slugs (1500 ms Sleep + 1–3 s API) ✅
...
### Welle 17A.3 — Bio-Hub-Seite /social (Juni 2026)

- **W17A.3.1** (Commit d12ec02) — `app/social/page.tsx` als Server Component mit `force-dynamic`, noindex/nofollow, Mobile-first-Layout (Logo, „Heute auf Instagram"-Block mit Kategorie-Farbe, Top-10 aus EXCLUDED_SLUGS, Footer). `lib/social/state.ts` um `setCurrentBioSlug` + `getCurrentBioSlug` erweitert. `slug-drift-scan` META_ROUTES um `social` ergänzt ✅
- **W17A.3.2** (Commit 3bc330d) — Publisher-Hook in `publishToOne`: nach erfolgreichem IG-Post wird `social:current-bio-slug = post.slug` gesetzt (bewusst nur IG, FB braucht den Pointer nicht). Best-effort KV-Write, Plattform-Erfolg bleibt unabhängig von KV-Fehler

### Methodische Lehren (NEU, Welle 17A.4 — KV-Host-Disziplin + force-Werkzeug)

- **L-W17A.NEU — KV-Host-Disziplin (zwei aktive DBs):** rechenfix nutzt `rechenfix-stats` (social-loon-99506), via Vercel-Integration verwaltet. `erklaerfix` (apt-tahr-124200) ist ein EIGENES, aktives Projekt von Karsten — **NICHT verwaist, NICHT löschen, NICHT umbenennen.** Risiko: Die Upstash-Console „Personal" kann beim Arbeiten an rechenfix prominent die falsche DB (erklaerfix) zeigen. Daher: (1) Für rechenfix-KV AUSSCHLIESSLICH über Vercel → rechenfix → Storage → rechenfix-stats → CLI arbeiten. (2) Vor jedem KV-Eingriff Host gegenchecken (`new URL(KV_REST_API_URL).host` bzw. im CLI-Kontext bestätigen, dass social-loon-99506 aktiv ist). (3) Symptom „KV-Wert/DEL kommt nicht an": zuerst den tatsächlich genutzten KV-Host prüfen, BEVOR Lese-/Schreib-Code debuggt wird. **Konkreter Vorfall:** IG-Doppelpost (06.06.2026) entstand, weil ein manueller `DEL social:done:autokosten-rechner:instagram` den falschen Store traf (erklaerfix statt rechenfix-stats) und `force=true` die noch stehende IG-Done-Marke überfuhr.

- **L-W17A force-Werkzeug:** `?force=true` umgeht BEIDE Idempotenz-Checks (per-Plattform Done-Marke `social:done:{slug}:{platform}` + same-day `social:posted:…`). Es postet bedingungslos auf BEIDEN Plattformen. **NICHT für Reposts verwenden, wenn eine Plattform bereits erledigt ist** (sonst Doppelpost auf der erledigten Plattform). Für gezielten Nachhol einer einzelnen Plattform: nur deren Done-Marke offen lassen und OHNE force den regulären Pfad nutzen, ODER force künftig per-Plattform-aware machen (Backlog). Der reguläre 19:00-Cron (ohne force) ist idempotent und sicher vor Doppelposts — er holt bei partial failure nur die fehlende Plattform nach.

> **TikTok-Takt (W52):** TikTok postet nur jeden zweiten Tag, gerechnet in ganzen Tagen ab `TIKTOK_TAKT_START` (06.08.2026) in `lib/social/utils.ts`. Grund ist das Post-Kontingent des Gratistarifs (20 je Abrechnungszyklus) — anbieterunabhängig, galt bei PostPeer wie bei bundle.social. Der Zyklus läuft ab Registrierungsdatum, nicht kalendarisch. IG/FB bleiben täglich. Der Takt wird über `tiktokEnabled()` in `lib/social/state.ts` gesteuert und wirkt darüber auf `platformsForSlug` — die Zuständigkeitslogik ist damit tagesabhängig. Wird der Takt geändert, ist **nur** `TIKTOK_TAKT_START` bzw. der Modulo-Wert anzufassen, nichts an der Rotationslogik.

> **Getrennte Cron-Zeiten (W53):** IG/FB posten um 17 UTC über `/api/cron/social-post`, TikTok um 05 UTC über `/api/cron/social-post-tiktok`. Grund ist der 24-Stunden-Creator-Cap von PostPeers API-Client, der sich über alle Kunden dieses Anbieters teilt — 17 UTC fällt in dessen Stoßzeit. Die Verschiebung war ein Messaufbau; sie half nicht, der Wechsel auf bundle.social (W59) schon. **W66:** Erster Post am 08.08.2026 sichtbar, kein Fehlereintrag. Auswertung ab 20.08. nach `docs/audit-arbeitspapiere/arbeitspapier-tiktok-messvorschrift-2026-08-08.md`. **W104:** Test vorzeitig beendet — 08.–18.08. sechs von sechs Slots gepostet, keine Lücke. Der PostPeer-Pfad ist entfernt; `TIKTOK_PROVIDER` kennt nur noch `bundle` (Default) und `direct`, jeder andere Wert bricht mit `PROVIDER_UNKNOWN` ab. `POSTPEER_API_KEY`, `POSTPEER_ACCOUNT_ID` und `TIKTOK_VIA_WRAPPER` in Vercel löschen. `vercel.json` enthält bewusst **keine** Query-Parameter im Cron-Pfad, weil Vercel diese nicht dokumentiert. Der Plattformfilter läuft über `publishToBothPlatforms(force, dryRun, platforms)`; der manuelle Re-Trigger `/api/cron/social-post?platforms=…` kann die Zuständigkeit überschreiben.

### Methodische Lehren (NEU, Welle 17A.3)

- **L-W17A.3.1: IG-Caption-Links sind nicht klickbar — Bio-Hub statt URL-im-Text.** Instagram macht aus URLs im Caption-Text reinen Anzeige-Text, nur der EINE Bio-Link ist klickbar. Naive Lösung „Bio-Link pro Post manuell ändern" skaliert nicht. Lösung: feste Bio-Hub-Seite (`/social`), die per KV-Read dynamisch den aktuell geposteten Slug oben zeigt — Pipeline schreibt nach IG-Erfolg `social:current-bio-slug`, Seite rendert serverseitig mit `force-dynamic`. **Pattern für vergleichbare Plattform-Restriktionen:** wenn die Plattform die Verlinkung beschränkt, NICHT die externe Quelle pflegen, sondern eine eigene Index-Seite vorhalten + serverseitig per Cron/State auf den richtigen Inhalt verzweigen. Skalierungs-Vorteil: 1× Bio-Link setzen, dann nie wieder ändern.
...
- **L-W17A.1.2: Done-Marken pro Slug entkoppeln Pipeline-Logik von Datumssequenzen.** Die ursprüngliche W17A-Logik `(today − startDate) mod len` war elegant kompakt, aber bei „jeder Slug genau einmal"-Anforderung gebrochen: bei Cron-Aussetzer (Vercel-Outage, Plattform-Pause) verschiebt sich der ganze Plan, manuelle Re-Trigger doppeln. W17A.1 macht das pro-Slug: KV-Key `social:done:{slug}` = „durch", Pipeline iteriert in Queue-Reihenfolge bis erstes un-done. **Pattern:** wenn „jedes Item genau einmal" gefordert ist, immer Item-basierte Done-Marken, nie Index/Datum-basierte Rotation.
- **L-W17A.1.3: Script-TypeScript-Constraints aus tsconfig.json fangen Build-Fehler erst lokal nach `npm run build`, nicht beim Editieren.** Zwei Stolperer im T7-Sprint: (a) `[...mySet]` als Spread braucht `downlevelIteration` oder `target: es2015+` — Fix via `Array.from(mySet)`. (b) Top-level-`await` braucht `module: es2022+` — Fix via `async function main(); main().catch()`. Beide Errors zeigen sich erst beim Prebuild-Hook, nicht beim TS-Lint im Editor. **Pattern:** in `scripts/`-Files diese zwei Idiome bewusst vermeiden, oder bei Bedarf tsconfig erweitern (nicht in Welle 17A.1 gemacht — nicht-invasiver Fix bevorzugt).
